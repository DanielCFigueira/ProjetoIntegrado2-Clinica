    ...
    <form action="<?= BASE_URL ?>usuarios/salvar" method="POST" style="margin-top: 1rem;">
        ...
        <div class="form-group">
            ...
            <select name="perfil" id="perfil" required>
                <?php $perfil = $usuario['perfil'] ?? ''; ?>
                <option value="recepcionista" <?= $perfil === 'recepcionista' ? 'selected' : '' ?>>Recepcionista</option>
                <option value="dentista" <?= $perfil === 'dentista' ? 'selected' : '' ?>>Dentista</option>
                <option value="proprietario" <?= $perfil === 'proprietario' ? 'selected' : '' ?>>Administrador</option>
            </select>
        </div>

        <div style="margin-top: 2rem;">
            <button type="submit" class="btn btn-success">Salvar Alterações</button>
            <a href="<?= BASE_URL ?>usuarios" class="btn btn-cancel">Cancelar</a>
        </div>
    </form>
</div>
