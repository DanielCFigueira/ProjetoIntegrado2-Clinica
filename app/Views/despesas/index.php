...
<div class="card">
    ...
    <div class="card" style="margin-top: 2rem;">
        <h3>Nova Despesa</h3>
        <form action="<?= BASE_URL ?>despesas/salvar" method="POST">
        ...
        <tbody>
            <?php if (count($despesas) > 0): ?>
                <?php foreach ($despesas as $despesa): ?>
                    <tr>
                        <td data-label="Data" data-sort="<?= date('Y-m-d', strtotime($despesa['data_despesa'])) ?>"><?= date('d/m/Y', strtotime($despesa['data_despesa'])) ?></td>
                        ...
                        <td data-label="Ação">
                            <a href="<?= BASE_URL ?>despesas/excluir?id=<?= $despesa['id'] ?>" class="btn btn-danger btn-delete">Remover</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
